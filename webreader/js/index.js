...
//入口函数
    function main(){
        ...
        var readerModel=ReaderModel();//调用ajax入口函数

        var readerUI=ReaderBaseFrame($('#format_container'))//阅读器UI渲染函数调用 传入container
...
//init 方法用来完成整个请求数据以及请求数据后对数据的处理和在页面中的渲染
        var init = function(UIcallback){//页面渲染方法 参数为页面渲染的回调函数
            ...
            getFictionInfo(function(){  //先执行请求章节列表的json数据 然后在回调函数中执行请求章节内容的getCurChapterContent函数
                ...
                getCurChapterContent(Chapter_id,function(data){//先请求章节内容，然后在执行回调函数渲染请求到的内容在页面上
                    //to do
...
//请求所有章节信息
        var getFictionInfo=function(callback){
            $.get('data/chapter.json',function(){
                ...
                //获得章节信息的回调
                Chapter_id = arguments[0].chapters[1].chapter_id; //Charter_id 赋值为返回json对象的chapters属性 该属性是一个数组数组成员为json格式的对象，这个对象的chapter_id属性值是数字 代表的是第几章
        ...
        var getCurChapterContent=function(chapter_id,callback){
            ...
            $.get('data/data'+chapter_id+'.json',function(data){//用get方法获取要获取的小说内容  参数1为 字符串拼接的路径，第二个参数为回掉函数在$.get执行完以后 获得到请求结果后 第三个参数为规定的请求结果的类型为json格式
                ...
                if(data.result==0){     //验证该json对象内的属性 如果验证成功说明data被成功的请求到了
                    ...
                    Util.getJSONP(url,function(data){ //调用jsonp方法

                        callback&&callback(data) //调用getCurChapterContent 方法的回调函数参数为data也就是通过获取章节内容得到的返回结果
                    })
                }
            },'json')
        }
        return{//返回一个对象，这样的好处是返回值拥有命名空间 只有通过ReaderModel.init属性才能访问到init函数体
            init:init
        }
    }
